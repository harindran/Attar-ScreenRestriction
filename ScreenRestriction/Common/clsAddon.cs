...
using ScreenRestriction.Business_Objects;
...
namespace ScreenRestriction.Common
{
    class clsAddon
    {
        ...
        public SAPbouiCOM.Form frmdataselectform;
        public int dataselRow = 0;
        public bool bModal = false;
        //public bool HANA = true;
        public bool HANA = false;

        public string[] HWKEY=   { "L1653539483", "X1211807750", "U0558002153" };
        ...
        public void Intialize(string[] args)
        {
            try
            {
                ...
                //HANA = Convert.ToBoolean(ConfigurationManager.AppSettings.Get("HANA"));
                //HWKEY= ConfigurationManager.AppSettings.Get("HWKEY").Split(',');
        ...
        public void Add_Authorizations()
        {
            try
            {
                ...
                clsModule.objaddon.objglobalmethods.AddToPermissionTree("Custom Form Settings Setup", "ATPL_SETUP", "USRSCRN", "ATPL_ADD-ON", 'Y');
        ...
        {
            int Menucount = 1;
            if (objapplication.Menus.Item("43520").SubMenus.Exists("USRSCRN") )
                return;
            //Menucount = 1;// objapplication.Menus.Item("8448").SubMenus.Count;
            Menucount = objapplication.Menus.Item("8448").SubMenus.Count;

            CreateMenu("", Menucount, "User Configuration", SAPbouiCOM.BoMenuType.mt_STRING, "USRSCRN", "8448"); Menucount += 1;
                    ...
                    {                        
                        ...
                        switch (pVal.EventType)
                        {
                            ...
                            case SAPbouiCOM.BoEventTypes.et_CLICK:
                                {
                                    if (clsModule.objaddon.bModal==true &  clsModule.objaddon.objapplication.Forms.ActiveForm.TypeEx== "USRSCRN")
                                    {
                                        BubbleEvent = false;
                                        clsModule.objaddon.objapplication.Forms.Item("DATASEL").Select();
                                    }
                                    break;
                                }
                            case SAPbouiCOM.BoEventTypes.et_FORM_CLOSE:
                                {                           
                                   ...
                                   if (FormUID == "DATASEL" & EventEnum == SAPbouiCOM.BoEventTypes.et_FORM_CLOSE )
                                    {
                                      clsModule.objaddon.bModal = false;
                                    }
                                    break;
                                }
                ...
                else
                {
                    ...
                    {                        
                        case SAPbouiCOM.BoEventTypes.et_FORM_LOAD:
                            {
                                string objtype =  objapplication.Forms.ActiveForm.BusinessObject.Type.ToString();
                                if (clsModule.objaddon.HANA == true)
                                {
                                    strsql = "Select T0.\"U_FieldName\" from \"@AT_USRSCRN1\" T0 Left Join \"@AT_USRSCRN\" T1 On T0.\"Code\"=T1.\"Code\"";
                                    strsql += "\n Where T0.\"Code\"='"+ clsModule.objaddon.objcompany.UserName + "' and T0.\"U_Active\"='Y' and T0.\"U_FieldName\"<>'' and T0.\"U_ObjType\"='"+ objtype + "'";
                                }
                                else
                                {
                                    strsql = "Select T0.U_FieldName from [@AT_USRSCRN1] T0 Left Join [@AT_USRSCRN] T1 On T0.Code=T1.Code";
                                ...
                                if ( objform.Title.ToUpper()== "LIST OF ITEMS")//objform.BusinessObject.Type.ToString() == "-1" &
                                {
                                    SAPbouiCOM.Matrix objmatrix;
                                    objmatrix =(SAPbouiCOM.Matrix) objform.Items.Item("7").Specific;
                                    for (int i = 0; i < objrs.RecordCount; i++)
                                    {
                                        for (int matcol = 1; matcol < objmatrix.Columns.Count-1; matcol++)
                                        {
                                            if (objmatrix.Columns.Item(matcol).UniqueID==objrs.Fields.Item(0).Value.ToString())
                                            {
                                                if (objmatrix.Columns.Item(matcol).Visible == true) objmatrix.Columns.Item(matcol).Visible = false; break;
                                            }
                                        }
                                        objrs.MoveNext();
                                    }
            ...
            try
            {
                if (pVal.BeforeAction == false)
                {
                    switch (pVal.MenuUID)
                    {
                        case "USRSCRN":                        
                            {
                                FrmUserConfiguration activeform = new FrmUserConfiguration();
                                activeform.Show();
                                break;
                            }
        ...
        private void DisConnect_Addon(string MenuID)
        {
            try
            {
                if (clsModule.objaddon.objapplication.Forms.Count > 0)
                {
                    try
                    {
                        ...
                        {
                            if (clsModule.objaddon.objapplication.Forms.Item(frm).IsSystem == true)
                                continue;
                    ...
                    if (MenuID != "") {
                        if (clsModule.objaddon.objapplication.Menus.Item("43520").SubMenus.Exists(MenuID))
                            clsModule.objaddon.objapplication.Menus.Item("43520").SubMenus.RemoveEx(MenuID); 
                    }
                    //clsModule.objaddon.objapplication.Menus.Item    ("")

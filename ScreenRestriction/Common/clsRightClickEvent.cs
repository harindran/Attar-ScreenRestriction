...
namespace ScreenRestriction.Common
{
    class clsRightClickEvent
    {
        ...
        public void RightClickEvent(ref SAPbouiCOM.ContextMenuInfo eventInfo, ref bool BubbleEvent)
        {
            try
            {
                ...
                switch (clsModule.objaddon.objapplication.Forms.ActiveForm.TypeEx)
                {
                    case "USRSCRN":
                        {
                            UserConfiguration_RightClickEvent(ref eventInfo,ref BubbleEvent);
                            break;
                        }
        ...
        private void UserConfiguration_RightClickEvent(ref SAPbouiCOM.ContextMenuInfo eventInfo, ref bool BubbleEvent)
        {
            try
            {
                ...
                if (eventInfo.BeforeAction)
                {
                    ...
                    if (eventInfo.ItemUID == "") objform.EnableMenu("1283", true); else objform.EnableMenu("1283", false);
                    objform.EnableMenu("1286", false);
                    
                    try
                    {
                        if (objform.Mode != SAPbouiCOM.BoFormMode.fm_FIND_MODE & eventInfo.ColUID=="#") objform.EnableMenu("1293", true);else objform.EnableMenu("1293", false);// Remove Row Menu                                
                                                                                                                                                       // Copy Table
                        if (Matrix0.Item.Type == SAPbouiCOM.BoFormItemTypes.it_MATRIX)
                        {
                            if (eventInfo.ItemUID== "mtxlist") objform.EnableMenu("784", true); //Copy Table
                            ...
                            {
                                if (((SAPbouiCOM.ComboBox)Matrix0.Columns.Item(eventInfo.ColUID).Cells.Item(eventInfo.Row).Specific).Selected.Value != "")                                
                                    objform.EnableMenu("772", true); // Copy                            
                                else
                                    objform.EnableMenu("772", false); // Copy 
                            }                               
